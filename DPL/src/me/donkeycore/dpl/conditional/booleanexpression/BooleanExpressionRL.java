...
final class BooleanExpressionRL extends BooleanExpression {
	 ...
	 */
	BooleanExpressionRL(final String newBooleanExpression)
		throws MalformedBooleanException {
		...
		switch (firstChar) {
			...
			case '(':
				...
				String closeToEnd = getFromCloseParenthesisToEnd(substring,
					index + 1);
				IBoolean boolBeginToClose= toIBoolean(
	...
	private IBoolean toIBoolean(final IBoolean lastIBoolean,
		...
		switch (firstChar) {
			...
			case ')':
				return toIBoolean(lastIBoolean, substring, index + 1);
			case '|':
				return new BooleanOrOperation(lastIBoolean, toIBoolean(
					substring, index + 2));
			case '&':
				return new BooleanAndOperation(lastIBoolean, toIBoolean(
					substring, index + 2));
			default:
				throw new MalformedBooleanException(
					"Expected [ ' ', ), ||, && ]", index,
	...
	/**
	 * Returns the supplied formated boolean expression without his first
