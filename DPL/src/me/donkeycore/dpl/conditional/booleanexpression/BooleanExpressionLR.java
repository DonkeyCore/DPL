...
final class BooleanExpressionLR extends BooleanExpression {
	 ...
	 */
	BooleanExpressionLR(final String newBooleanExpression)
		throws MalformedBooleanException {
		this.booleanExpression = newBooleanExpression;
		this.iBoolean = toIBoolean(BooleanUtil
	 ...
	 *            The formated boolean expression to transform to
	 *            {@link IBoolean}.
	 * @param index
	 ...
	 */
	private IBoolean toIBoolean(final String formatedBooleanExpression,
		final int index) throws MalformedBooleanException {
		...
		switch (lastChar) {
			...
			case ')':
				...
				IBoolean boolToClose = toIBoolean(boolOpenToEnd, beginToOpen,
					index - 1);
				return boolToClose;
			case 'T':
				IBoolean boolTrue = toIBoolean(new Boolean(true), substring,
					index - 4);
				return boolTrue;
			case 'F':
				IBoolean boolFalse = toIBoolean(new Boolean(false), substring,
					index - 5);
				return boolFalse;
			default:
	 ...
	 *             If the supplied formated boolean expression is malformed.
	 */
	private IBoolean toIBoolean(final IBoolean lastIBoolean,
		...
		switch (lastChar) {
			case ' ':
				IBoolean boolWhitespace = toIBoolean(lastIBoolean, substring,
					index - 1);
				return boolWhitespace;
			case '.':
				return lastIBoolean;
			case '(':
				IBoolean boolToOpen = toIBoolean(lastIBoolean, substring,
					index - 1);
				return boolToOpen;
			case '|':
				IBoolean boolFirstOr = toIBoolean(substring, index - 2);
				IBoolean boolOr = new BooleanOrOperation(boolFirstOr,
					lastIBoolean);
				return boolOr;
			case '&':
				IBoolean boolFirstAnd = toIBoolean(substring, index - 2);
				IBoolean boolAnd = new BooleanAndOperation(boolFirstAnd,
					lastIBoolean);
				return boolAnd;
			case '!':
	 ...
	 * expression, or '.' if the supplied formated boolean expression is
	 * <code>null</code> or void.
	 * 
	 * @param formatedBooleanExpression
	 *            The formated boolean expression to get the first
	 *            <code>char</code>.
	...
	private char getLastChar(final String formatedBooleanExpression) {
		...
		return formatedBooleanExpression.charAt(formatedBooleanExpression
			.length() - 1);
	}

	/**
	 * Returns the supplied formated boolean expression without his last
	 * <code>char</code>, or "" if the supplied formated boolean expression
	...
	private String getSubstringWithoutLastChar(
		...
		return formatedBooleanExpression.substring(0, formatedBooleanExpression
			.length() - 1);
	}
	 ...
	 *         parenthesis, or "" if the supplied formated boolean expression is
	 *         <code>null</code> or void.
	 * @throws MalformedBooleanException
	 *             If the supplied formated boolean expression is malformed.
	 */
